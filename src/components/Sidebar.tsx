    ...
    <aside className={`${collapsed ? 'w-16' : 'w-64'} flex-shrink-0 bg-white border-r border-surface-border flex flex-col transition-all duration-300 relative z-20 shadow-sm`}>
      ...
      <div className="flex items-center gap-3 px-4 py-4 border-b border-surface-border">
        ...
        <button
          ...
          className="ml-auto text-brand-grey hover:text-primary transition-colors flex-shrink-0"
        >
      ...
      {/* System Status */}
      {!collapsed && (
        <div className="mx-3 mt-3 mb-1 px-3 py-2 bg-surface-elevated rounded-lg border border-surface-border">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-accent-green animate-pulse-slow"></div>
            <span className="text-xs text-brand-grey font-medium">All 9 Agents Online</span>
          </div>
        </div>
      )}
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {visibleNavItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`flex-shrink-0 ${isActive ? 'text-white' : 'text-brand-grey group-hover:text-primary'}`}>
                {item.icon}
              </span>
              {!collapsed && (
              ...
              {!collapsed && item.badge && (
                <span className={`ml-auto text-xs px-1.5 py-0.5 rounded-full font-mono ${isActive ? 'bg-white/20 text-white' : 'bg-primary/10 text-primary'}`}>
                  {item.badge}
                </span>
              )}
              {collapsed && (
                <div className="absolute left-full ml-2 px-2 py-1 bg-primary text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">
                  {item.label}
                </div>
              )}
            </Link>
      ...
      <div className="px-2 py-3 border-t border-surface-border space-y-1">
        {isAdmin && (
          <Link
            href="#"
            className="flex items-center gap-3 px-3 py-2 rounded-xl text-brand-grey hover:text-primary hover:bg-surface-elevated transition-all duration-200 group"
          >
            <svg className="w-5 h-5 flex-shrink-0 text-brand-grey group-hover:text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="px-3 py-2 mt-1">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-semibold text-brand-dark truncate">{profile.full_name}</div>
                <div className="text-xs text-brand-grey truncate capitalize">{profile.role.replace('_', ' ')}</div>
              </div>
              <button
                onClick={signOut}
                className="text-brand-grey hover:text-accent-red transition-colors flex-shrink-0"
                title="Sign out"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
